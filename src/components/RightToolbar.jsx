...
const RightToolbar = ({ settings, onSettingsChange, imageData }) => {
  ...
  const originalDimensions = imageData
    ? { width: imageData.width, height: imageData.height }
    : dimensions;

  const croppedWidth = Math.round(
    (originalDimensions.width * crop.width) / 100
  );
  const croppedHeight = Math.round(
    (originalDimensions.height * crop.height) / 100
  );

  return (
    <div className="flex-1 overflow-y-auto p-4 sm:p-6 bg-slate-800">
      <h3 className="text-white text-lg sm:text-xl font-bold mb-4 sm:mb-6 pb-3 border-b border-gray-600">
        Edit Controls
      </h3>

      {/* Resize Section */}
      <div className="mb-5 sm:mb-6 pb-5 sm:pb-6 border-b border-gray-600">
        <h4 className="text-white text-sm sm:text-base font-bold mb-3">
          Resize Image
        </h4>
        ...
        <select
          ...
          className="w-full p-3 sm:p-2.5 bg-gray-700 text-white rounded-lg border-2 border-gray-600
          ...
          <div className="space-y-2">
            <div className="flex flex-col sm:flex-row gap-2">
              <input
                ...
                className="flex-1 p-3 sm:p-2 bg-gray-700 text-white text-sm rounded-lg
                border-2 border-gray-600 focus:border-purple-400 focus:ring-2
                focus:ring-purple-400 focus:outline-none font-medium"
              />

              <span className="hidden sm:block text-white self-center text-sm font-bold">
                ×
              </span>
            ...
            <button
              ...
              className="w-full px-4 py-2 bg-purple-600 text-white text-sm font-bold
        ...
        {/* Image Info Table */}
        <div className="mt-3 text-xs bg-slate-900 rounded-lg p-3 border border-gray-700 overflow-x-auto">
      ...
      <div className="border-t border-gray-600 pt-4 mt-4">
        <h4 className="text-white text-sm sm:text-base font-bold mb-3 sm:mb-4">
          Crop Position
        </h4>
